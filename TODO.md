...
- 每次修改都执行维护影响检查；只有发现新的维护责任，或影响权威来源、比较资格、验收门、撤销条件、元数据、交叉引用时，才在同一变更中同步更新维护方式。无影响也要在 PR 中记录判断，但不因此新增字段、模板或自动化。
...
- **验收出口**：能直接指导 ROI 分类、局部关键点和轻量检测三类实验；用多分面覆盖常用候选而不制造互斥分类树；架构收益与预训练收益分开；训练前先做接口/算子可行性；最终采用由同协议任务结果与目标硬件 INT8 结果共同决定；每次变更都完成维护影响检查，发现新的、可复用的维护要求时同步修订本模块。
