...
本页保存跨检测器复用的因果链和诊断入口。不同检测范式如何重构候选、分配、结果选择与语义接口，见 [目标检测架构演变](object-detection-architecture-evolution.md)；某个 YOLO 分支采用什么模块、节点或源码实现，由 [YOLO family](../model-index/families/yolo/README.md) 维护。
...
对应任务定义：[目标检测](../tasks/object-detection.md)。跨范式关系见 [目标检测架构演变](object-detection-architecture-evolution.md)。本页的机制结构、推断边界和诊断入口已于 2026-07-27 通过本人审查；本次只补充关系入口，不改变原验收边界，接受不等于机制已在全部模型与任务中验证。
